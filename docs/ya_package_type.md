...
### Сборка tar-пакетов
Архивы в формате `tar` создаются по умолчанию, если не указать конкретный формат, но также можно использовать ключ `--tar`.
...
2. Необходимо добавить в файл-описание пакета в секцию `meta` зависимость от пакета `python-support` в список `pre-depends`, иначе на чистой машине пакет не подключится к `Python`, даже если файлы присутствуют.
...
- Значение: В `settings.xml` можно задать параметры, которые иначе передавались бы в `Maven` без использования `ya  package`
